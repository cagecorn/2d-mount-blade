...
export class Game {
    constructor(canvasId = 'map-base-canvas') {
        this.canvas = document.getElementById(canvasId);
        if (!this.canvas) {
            console.warn(`Canvas element with id '${canvasId}' not found.`);
            // 게임 로직이 캔버스 없이도 동작하도록 빈 캔버스를 생성한다
    ...
    async start() {
        ...
        const managers = {
            ...
            bgmManager: new Managers.BgmManager(),
    ...
    startBGM() {
        const bgm = context.bgmManager;
        if (bgm && !bgm.isInitialized) {
            bgm.start();
        }
    }
}
